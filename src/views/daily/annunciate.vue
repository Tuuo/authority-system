<template>
  <div class="approval-page">
    ...
    <system-dialog :title="approvalDialog.title" :visible="approvalDialog.visible" :width="approvalDialog.width"
      ...
      <div slot="content">
        <el-form :model="group" ref="groupFrom" label-width="80px" :inline="false" size="small" :rules="rules">
    ...
    <system-dialog :title="handledDialog.title" :visible="handledDialog.visible" :width="handledDialog.width"
      ...
      <div slot="content">
        <el-form :model="handled" ref="noFrom" label-width="80px" :inline="false" size="small" :rules="rules2">
          ...
          <el-form-item label="类型" prop="type">
            <el-select v-model="handled.type" placeholder="请选择" class="tasks" @change="onEducationSelectOption" style="width: 290px">
          ...
          <el-form-item label="发送方式" prop="send">
            <el-select v-model="handled.send" placeholder="请选择" class="tasks" @change="onSendSelectOption" style="width: 290px">
          ...
          <el-form-item label="回复方式" prop="reply">
            <el-select v-model="handled.reply" placeholder="请选择" class="tasks" @change="onReplySelectOption" style="width: 290px">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        groupName: [{required: true, trigger: 'change', message: '请输入分组名称'}],
        groupPerson: [{required: true, trigger: 'change', message: '请输入分组人员'}],
      },
      rules2: {
        name: [{required: true, trigger: 'change', message: '请输入接收人'}],
        title: [{required: true, trigger: 'change', message: '请输入通告标题'}],
        type: [{required: true, trigger: 'change', message: '请选择通告类型'}],
        content: [{required: true, trigger: 'change', message: '请输入内容'}],
        send: [{required: true, trigger: 'change', message: '请选择发送方式'}],
        reply: [{required: true, trigger: 'change', message: '请选择回复方式'}],
      },
      roleList: [], //数据列表
      tableHeight: 0, //表格高度
      pageNo: 1, //当前页码
