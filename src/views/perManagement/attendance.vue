<template>
  <div class="approval-page">
          ...
          <el-tabs v-model="activeTab" @tab-click="handleTabClick">
            ...
            <el-tab-pane label="加班申请" name="handled">
              <el-form :model="searchNoModel" ref="searchNoForm" label-width="80px" :inline="true"
                ...
                <el-form-item>
                  <el-button type="primary" icon="el-icon-search"
                             @click="searchNO(pageNo2, pageSize2)">查询
                  </el-button>
    ...
    <system-dialog :title="approvalDialog.title" :visible="approvalDialog.visible" :width="approvalDialog.width"
      ...
      <div slot="content">
        <el-form :model="group" ref="groupFrom" label-width="110px" :inline="false" size="small" :rules="rules">

          <el-form-item label="出差人员" prop="name">
            <el-input v-model="group.name" style="width: 220px"></el-input>
          </el-form-item>

          <el-form-item label="出差时间" prop="tripDate">
            <el-col :span="11">
          ...
          <el-form-item label="出差事由" prop="reason">
            <el-input v-model="group.reason" style="width: 220px"></el-input>
          </el-form-item>
          <el-form-item label="出差目的地" prop="location">
            <el-input v-model="group.location" style="width: 220px"></el-input>
          </el-form-item>
          <el-form-item label="出差天数" prop="tripDay">
            <el-input v-model="group.tripDay" style="width: 220px"></el-input>
          </el-form-item>
      ...
      <div slot="content">
        <el-form :model="handled" ref="noFrom" label-width="110px" :inline="false" size="small" :rules="rules2">
          ...
          <el-form-item label="姓名" prop="name">
            <el-input v-model="handled.name" style="width: 220px"></el-input>
          </el-form-item>

          <el-form-item label="部门" prop="department">
            <el-input v-model="handled.department" style="width: 220px"></el-input>
          </el-form-item>
          <el-form-item label="类型" prop="type">
            <el-select v-model="handled.type" placeholder="请选择" class="tasks" @change="onEducationSelectOption" style="width: 220px">
              <el-option v-for="item in typeOptions" :key="item.value" :label="item.label"
                         :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="加班开始时间" prop="start">
            <el-col :span="11">
          ...
          <el-form-item label="加班结束时间" prop="end">
            <el-col :span="11">
          ...
          <el-form-item label="加班原因" prop="reason">
            <el-input v-model="handled.reason" style="width: 220px"></el-input>
          </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        tripDate: [{required: true, trigger: 'change', message: '请输入出差时间'},
          { validator: this.validateStart, trigger: 'change' }],
        name: [{required: true, trigger: 'change', message: '请输入出差人员'}],
        reason: [{required: true, trigger: 'change', message: '请输入出差事由'}],
        location: [{required: true, trigger: 'change', message: '请输入出差目的地'}],
        tripDay: [{required: true, trigger: 'change', message: '请输入出差天数'}],
        // contain: [{required: true, trigger: 'change', message: '请选择工作状态'}],
        returnDate: [{required: true, trigger: 'change', message: '请输入返回时间'},
      ...
      rules2: {
        ...
        reason: [{required: true, trigger: 'change', message: '请输入加班原因'},],
        start: [{required: true, trigger: 'change', message: '请输入加班开始时间'},
          { validator: this.validateStart2, trigger: 'change' }],
        // contain: [{required: true, trigger: 'change', message: '请选择工作状态'}],
        end: [{required: true, trigger: 'change', message: '请输入加班结束时间'},
  ...
  methods: {
    validateStart(rule, value, callback) {
      ...
      if (start > end) {
        callback(new Error('出差时间不能晚于返回日期'));
      } else {
        callback();
      }
    },
    validateEnd(rule, value, callback) {
      const start = new Date(this.group.tripDate);
      const end = new Date(value);
      if (start > end) {
        callback(new Error('返回日期不能早于出差日期'));
      } else {
        callback();
      }
    },
