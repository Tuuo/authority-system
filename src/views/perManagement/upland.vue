  ...
  <el-container :style="{ height: containerHeight + 'px' }">
    ...
    <el-main>
            ...
            <el-table-column align="center" width="290" label="操作">
              <template slot-scope="scope">
                ...
                <el-button icon="el-icon-setting" type="primary" size="mini"
                           @click="assignRole(scope.row)">分配角色
                </el-button>
              </template>
            </el-table-column>
          </el-table>
      ...
      <system-dialog :title="holidayDialog.title" :visible="holidayDialog.visible" :width="holidayDialog.width"
        ...
        <div slot="content">
          <el-form :model="holiday" ref="holidayFrom" label-width="100px" :inline="false" size="small" :rules="rules3">
            <el-form-item label="节假日名称" prop="name" >
              <el-input v-model="holiday.name" style="width: 220px"></el-input>
            </el-form-item>
      ...
      <system-dialog :title="approvalDialog.title" :visible="approvalDialog.visible" :width="approvalDialog.width"
        ...
        <div slot="content">
          <el-form :model="group" ref="groupFrom" label-width="120px" :inline="false" size="small" :rules="rules2">
            ...
            <el-form-item label="专业" prop="specialty">
              <el-input v-model="group.specialty"></el-input>
            </el-form-item>
            <el-form-item label="外聘开始时间" prop="ououtStart">
              <el-col :span="13">
                <el-date-picker type="date" placeholder="选择日期" v-model="group.ououtStart"
                                value-format="yyyy-MM-dd" style="width: 460px">
                </el-date-picker>
              </el-col>
            </el-form-item>
            <el-form-item label="外聘结束时间" prop="ououtEnd">
              <el-col :span="13">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules2: {
        ...
        school: [{required: true, trigger: 'change', message: '请输入学校'}],
        specialty: [{required: true, trigger: 'change', message: '请输入专业'}],
        ououtStart: [{required: true, trigger: 'change', message: '请输入外聘开始时间'},
          {validator: this.validateStart, trigger: 'change'}],
        ououtEnd: [{required: true, trigger: 'change', message: '请输入外聘结束时间'},
          {validator: this.validateEnd, trigger: 'change'}],
        // person: [{required: true, trigger: 'change', message: '请输入介绍人'}],
      ...
      rules3: {
        name: [{required: true, trigger: 'change', message: '请输入节假日名称'}],
        holidayDate: [{required: true, trigger: 'change', message: '请输入日期'}],
        status: [{required: true, trigger: 'change', message: '请选择工作状态'}],
        isHoliday: [{required: true, trigger: 'change', message: '请选择是否是节假日'}],
  ...
  methods: {
    ...
    validateStart(rule, value, callback) {
      ...
      const end = new Date(this.group.ououtEnd);
      if (start > end) {
    ...
    validateEnd(rule, value, callback) {
      const start = new Date(this.group.ououtStart);
    ...
    handleHolidayEdit(row) {
      ...
      //设置窗口标题
      this.holidayDialog.title = "编辑节假日信息"
      //显示编辑角色窗口
