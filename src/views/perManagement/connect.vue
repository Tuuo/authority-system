      ...
      <div slot="content">
        <el-form :model="group" ref="groupFrom" label-width="110px" :inline="false" size="small" :rules="rules">
          ...
          <el-form-item label="所在部门" prop="department">
            <el-input v-model="group.department" style="width: 220px"></el-input>
          </el-form-item>
          <el-form-item label="职务" prop="job">
            <el-input v-model="group.job" style="width: 220px"></el-input>
          </el-form-item>
          ...
          <el-form-item label="交接状态" prop="status">
            <el-input v-model="group.status" style="width: 220px"></el-input>
          </el-form-item>
      ...
      <div slot="content">
        <el-form :model="handled" ref="noFrom" label-width="100px" :inline="false" size="small" :rules="rules2">
          ...
          <el-form-item label="身份证号" prop="cardId">
            <el-input v-model="handled.cardId" style="width: 220px"></el-input>
          </el-form-item>
          <el-form-item label="专业" prop="specialty">
            <el-input v-model="handled.specialty" style="width: 220px"></el-input>
          </el-form-item>
          <el-form-item label="性别" prop="gender">
            <el-input v-model="handled.gender" style="width: 220px"></el-input>
          </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        department: [{required: true, trigger: 'change', message: '请输入所在部门'}],
        job: [{required: true, trigger: 'change', message: '请输入职务'}],
        personType: [{required: true, trigger: 'change', message: '请输入人员类别'}],
        leftType: [{required: true, trigger: 'change', message: '请输入离岗类别'}],
        status: [{required: true, trigger: 'change', message: '请输入交接状态'}],

      },
      rules2: {
        name: [{required: true, trigger: 'change', message: '请输入人员'},],
        department: [{required: true, trigger: 'change', message: '请输入部门'},],
        cardId: [{required: true, trigger: 'change', message: '请输入身份证号'},],
        specialty: [{required: true, trigger: 'change', message: '请输入专业'},],
        gender: [{required: true, trigger: 'change', message: '请输入性别'},],
        startDate: [{required: true, trigger: 'change', message: '请输入入职时间'},
          { validator: this.validateStart2, trigger: 'change' }],
        // contain: [{required: true, trigger: 'change', message: '请选择工作状态'}],
        endDate: [{required: true, trigger: 'change', message: '请输入离职时间'},
  ...
  methods: {
    ...
    exportExcel() {
      ...
      let ws = XLSX.utils.json_to_sheet(formattedData, {header: ['编号', '部门', '姓名',
          '身份证号', '专业', '性别', '入职日期','离职日期']});
    ...
    getFormattedData() {
      return this.multipleSelection.map(item => ({
        ...
        '专业': item.specialty,
        '性别': item.gender,
        '入职日期': new Date(item.startDate).toLocaleDateString(),
        '离职日期': new Date(item.endDate).toLocaleDateString()
      }));
    }
  },
  mounted() {
