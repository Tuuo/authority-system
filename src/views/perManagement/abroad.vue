<template>
  <div class="approval-page">
    ...
    <system-dialog :title="approvalDialog.title" :visible="approvalDialog.visible" :width="approvalDialog.width"
      ...
      <div slot="content">
        <el-form :model="group" ref="groupFrom" label-width="90px" :inline="false" size="small" :rules="rules">
          ...
          <el-form-item label="领取日期" prop="getDate">
            <el-col :span="11">
          ...
          <el-form-item label="领取人" prop="person">
            <el-input v-model="group.person" style="width: 220px"></el-input>
          </el-form-item>
          <el-form-item label="备注" prop="contain">
            <el-input v-model="group.contain" style="width: 220px"></el-input>
          </el-form-item>
    ...
    <system-dialog :title="handledDialog.title" :visible="handledDialog.visible" :width="handledDialog.width"
      ...
      <div slot="content">
        <el-form :model="handled" ref="noFrom" label-width="110px" :inline="false" size="small" :rules="rules2">

          <el-form-item label="标题" prop="title" >
            <el-input v-model="handled.title" style="width: 220px"></el-input>
          </el-form-item>
          ...
          <el-form-item label="上报截止时间" prop="upEnd">
            <el-col :span="11">
    ...
    <system-dialog :title="giftDialog.title" :visible="giftDialog.visible" :width="giftDialog.width"
      ...
      <div slot="content">
        <el-form :model="gift" ref="hoFrom" label-width="110px" :inline="false" size="small" :rules="rules3">
          ...
          <el-form-item label="礼品期" prop="giftDate">
            <el-col :span="11">
          ...
          <el-form-item label="历史未领取人" prop="person">
            <el-input v-model="gift.person" style="width: 220px"></el-input>
          </el-form-item>
      ...
      <div slot="content">
        <el-form :model="abroad" ref="aoFrom" label-width="110px" :inline="false" size="small"  :rules="rules4">
          ...
          <el-form-item label="类型" prop="type">
            <el-select v-model="abroad.type" placeholder="请选择" class="tasks" @change="onEducationSelectOption" style="width: 220px">
          ...
          <el-form-item label="目的地" prop="location">
            <el-input v-model="abroad.location" style="width: 220px"></el-input>
          </el-form-item>
        </el-form>
      </div>
    </system-dialog>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        getDate: [{required: true, trigger: 'change', message: '请输入领取日期'},
          { validator: this.validateStart, trigger: 'change' }],
        person: [{required: true, trigger: 'change', message: '请输入领取人'}],
        // contain: [{required: true, trigger: 'change', message: '请选择工作状态'}],
        returnDate: [{required: true, trigger: 'change', message: '请输入归还日期'},
          { validator: this.validateEnd, trigger: 'change' }],
      },
      rules2: {
        title: [{required: true, trigger: 'change', message: '请输入标题'},
        ],
        start: [{required: true, trigger: 'change', message: '请输入开始时间'},
          { validator: this.validateStart3, trigger: 'change' }],
        // contain: [{required: true, trigger: 'change', message: '请选择工作状态'}],
        upEnd: [{required: true, trigger: 'change', message: '请输入上报截止时间'},
          { validator: this.validateEnd3, trigger: 'change' }],
      },
      rules3: {
        name: [{required: true, trigger: 'change', message: '请输入礼物名称'},
        ],
        giftDate: [{required: true, trigger: 'change', message: '请输入礼物期'},],
        // contain: [{required: true, trigger: 'change', message: '请选择工作状态'}],
        person: [{required: true, trigger: 'change', message: '请输入历史未领取人'},],
      },
      rules4: {
        name: [{required: true, trigger: 'change', message: '请输入姓名'},],
        type: [{required: true, trigger: 'change', message: '请输入类型'},],
        start: [{required: true, trigger: 'change', message: '请输入开始时间'},
          { validator: this.validateStart2, trigger: 'change' }],
        end: [{required: true, trigger: 'change', message: '请输入结束时间'},
          { validator: this.validateEnd2, trigger: 'change' }],
        reason: [{required: true, trigger: 'change', message: '请选择原因'}],
        location: [{required: true, trigger: 'change', message: '请选择目的地'}],
  ...
  methods: {
    validateStart(rule, value, callback) {
      ...
      const end = new Date(this.group.returnDate);
      if (start > end) {
        callback(new Error('领取日期不能晚于归还日期'));
      } else {
        callback();
      }
    },
    validateEnd(rule, value, callback) {
      const start = new Date(this.group.getDate);
      const end = new Date(value);
      if (start > end) {
        callback(new Error('归还日期不能早于领取日期'));
      } else {
        callback();
      }
    },
    validateStart3(rule, value, callback) {
      const start = new Date(value);
      const end = new Date(this.handled.start);
      if (start > end) {
        callback(new Error('上报日期不能晚于截止日期'));
      } else {
        callback();
      }
    },
    validateEnd3(rule, value, callback) {
      const start = new Date(this.handled.upEnd);
      const end = new Date(value);
      if (start > end) {
        callback(new Error('截止日期不能早于上报日期'));
      } else {
        callback();
      }
    },
    validateStart2(rule, value, callback) {
      const start = new Date(value);
      const end = new Date(this.abroad.end);
      if (start > end) {
    ...
    validateEnd2(rule, value, callback) {
      const start = new Date(this.abroad.start);
      const end = new Date(value);
      if (start > end) {
    ...
    notiEdit(row) {
      ...
      //设置窗口标题
      this.handledDialog.title = "编辑单据审批"
      //显示编辑角色窗口
    ...
    AotiEdit(row) {
      ...
      //设置窗口标题
      this.abroadDialog.title = "编辑出境申请"
      //显示编辑角色窗口
